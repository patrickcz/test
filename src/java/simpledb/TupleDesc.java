...
public class TupleDesc implements Serializable {
    ...
    public TupleDesc(Type[] typeAr) {
        ...
        for(Type type : typeAr){
        	TDItem newItem = new TDItem(type, "");
        	this.TDItemList.add(newItem);
        }
    }
    ...
    public Type getFieldType(int i) throws NoSuchElementException {
        ...
        if (i<0 || i>=TDItemList.size()){
        	throw new NoSuchElementException("position" + i + "is invalid.");	
        }
    ...
    public int fieldNameToIndex(String name) throws NoSuchElementException {
        // some code goes here      
        if(name == null){
            throw new NoSuchElementException("Feild name" + name + "is not founded.");
        }
    ...
    public static TupleDesc merge(TupleDesc td1, TupleDesc td2) {
        ...
        Type[] typeList = new Type[numFields];
        String[] nameList = new String[numFields];
        
        int index = 0;
        for (TDItem item : td1.TDItemList){
            typeList[index] = item.fieldType;
            nameList[index] = item.fieldName;
            index ++;
        }
    ...
    public boolean equals(Object o) {
        // some code goes here     
        if(!this.getClass().isInstance(o) || o == null){
            return false;
        }
        
        TupleDesc obj = (TupleDesc) o;
        int thisLen = this.numFields();
        int objLen = obj.numFields();
        
        if(thisLen != objLen) return false;
        
        for(int i=0; i<thisLen; i++){
            TDItem thisItem = this.TDItemList.get(i);
            TDItem objItem = obj.TDItemList.get(i);
            if(!thisItem.fieldType.equals(objItem.fieldType)){
                return false;
            }
        }
